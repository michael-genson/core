"""Tests for the todo integration intents."""
...
async def test_complete_item_intent(
    ...
    await entity1.async_create_todo_item(TodoItem(summary="beer", uid="1"))
    await entity1.async_create_todo_item(TodoItem(summary="cheese", uid="2"))
    await entity2.async_create_todo_item(TodoItem(summary="wine", uid="3"))

    await create_mock_platform(hass, [entity1, entity2])

    # Complete on first list
    response = await intent.async_handle(
        hass,
        "test",
    ...
    seen_items = set()
    for item in entity1.items:
        seen_items.add(item.summary)
        if item.summary == "beer":
            assert item.status == TodoItemStatus.COMPLETED
    assert seen_items == {"beer", "cheese"}

    # Complete on second list
    response = await intent.async_handle(
        hass,
        "test",
        todo_intent.INTENT_LIST_COMPLETE_ITEM,
    ...
    assert entity2.items[0].summary == "wine"
    assert entity2.items[0].status == TodoItemStatus.COMPLETED
    ...
    response = await intent.async_handle(
        ...
        "test",
        todo_intent.INTENT_LIST_COMPLETE_ITEM,
        {ATTR_ITEM: {"value": "cheese"}, "name": {"value": "lIST 1"}},
        assistant=conversation.DOMAIN,
    )
    assert response.response_type == intent.IntentResponseType.ACTION_DONE
